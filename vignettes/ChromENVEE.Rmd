...
**ChromENVEE** (**C**hromatin **ENV**rironment and **E**nhancer-dependent **E**xpression) is a package initially developed to define chromatin dynamics during spermatogenesis and to characterize H3K79me2 at the enhancer level and its chromatin environment. Several recent studies using ChromHMM have shown that H3K79me2 may have a role on gene expression when associated with enhancers (*Ferrari et al*; *Godfrey et al*)
...
The function takes a few minutes to process depending on the length of your enhancer table. It is possible to multithread the function with the `nCore` parameter. For each enhancer position, we get two pieces of information, the name of the associated gene and its distance to the enhancer (in bp).
...
`enhancerExpression()` able to associate the level of gene expression to each gene-enhancer pair taht is determined by the `enhancerAnnotation` function.
It is possible that a gene-enhancer pair does not have an expression level. In this case, the function returns NA value.
...
### Gene expression associated with the enhancers according to gene-enhancer distance

`plotDistanceExpression()` able to generate a plot to visualize the level of gene expression according to the distance between a gene and the enhancer to which it is associated. The distance is estimated with `limit` argument and clusterized in 6 distance groups as visualized in the plot below.
...
### Distribution of gene according to the distance between gene and enhancer

`plotGeneDistance()` able to generate a plot to visualize gene distribution according to the distance between a gene and the enhancer to which it is associated. The distance is estimated with `limit` argument and clusterized in 6 distance groups as visualized in the plot below.
...
### Expression of a gene associated with the enhancer

`plotEnhancerExpression()` able to generate a plot to visualize the gene expression distribution associated with the enhancer. It is possible to rescale the plot with the `scale` argument ('none','log10' and 'log2' are accepted).
...
### Expression of gene associated with the enhancer according to their distance

We generated a plot to visualize the level of gene expression according to the distance between a gene and the enhancer to with which it is associated, using `plotDistanceExpression`. The distance is estimated with a `limit` argument and clusterized in 6 distance groups as visualized in the plot below. In the case of list analysis, the function shows the average expression associated with each enhancer.
...
`plotGeneDistance()` able to generate a plot to visualize gene distribution according to the distance between a gene and the enhancer with which it is associated. The distance is estimated with a `limit` argument and clusterized in 6 distance groups as visualized in the plot below.
...
Crossing information from ChIPseq and RNAseq data provides information at the gene body level. The objective is to look at what chromatin information can emerge when we are interested in the chromatin environment of a gene. To do this, we need gene information data (`geneExpression`) and chromatin states data (`chromatinState`).
...
`geneEnvironment()` is a function able to estimate the chromatin state environment of the gene. To do this, we estimate an environment with an `interval` parameter.
...
It is an example of UMAP representation to visualize the predominant chromatin state in each gene. Each point corresponds to a gene and is colored according to its predominant chromatin state. The representation of the figure may not be exactly the same as the one presented in this thumbnail. Indeed, the calculation of the dimensions may differ, however the clusters remain the same.
