...
It's possible to comparated different categories of enhancer. For that it's necessary to use a list of GRanges file each contains data like `chromatin_state` file.
...
We estimated the coverage of each chromatin state categories in the environment of each gene. We estimated as environment the interval around the TSS with `interval` argument.
```{r}
